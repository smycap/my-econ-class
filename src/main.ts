...
// 최신 DB 로드 함수
function getDB() {
  return JSON.parse(localStorage.getItem('econ_v17_db') || JSON.stringify({
    globalRoles: [],
    globalStudents: [],
    weeklyActivity: {}, 
    treasury: { totalTax: 0 },
...
// 새로고침 시 로그인 유지를 위한 세션 스토리지
let currentUser: any = JSON.parse(sessionStorage.getItem('econ_user') || 'null');
const DAYS = ['월', '화', '수', '목', '금'];
const APP_TITLE = "🏛️ 민영쌤의 경제교실"; 

const app = document.querySelector<HTMLDivElement>('#app')!;

function getWeeklyActivity(viewKey: string, currentDb: any) {
  if (!currentDb.weeklyActivity[viewKey]) {
    currentDb.weeklyActivity[viewKey] = { checks: {}, isPaid: false, dayLocks: [false, false, false, false, false] };
  }
  return currentDb.weeklyActivity[viewKey];
}
...
function render() {
  ...
  const activity = getWeeklyActivity(currentView, db);
  ...
  app.innerHTML = `
    <div style="padding: 20px; font-family: sans-serif; max-width: 1100px; margin: auto;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h1 style="margin:0; font-size: 1.2rem;">${APP_TITLE}</h1>
        <button onclick="location.reload()" style="padding: 8px 15px; cursor: pointer; background: #e7f5ff; border: 1px solid #339af0; border-radius: 5px; font-weight: bold; color:#1864ab;">🔄 데이터 동기화</button>
      </div>
      ...
      <div style="overflow-x: auto; margin-top:30px; border-radius: 10px; border: 1px solid #dee2e6;">
        <table style="width: 100%; border-collapse: collapse; background: white; table-layout: fixed;">
          <thead style="background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
            <tr>
              <th style="padding:15px; width: 80px;">부서</th>
              <th style="padding:15px; width: 80px;">이름</th>
              <th style="padding:15px; width: 140px;">직업 (주급)</th>
              ${DAYS.map((d, i) => `
                <th style="padding:10px; width: 80px;">
                  ${d}<br>
                  ${currentUser.isAdmin ? `<button onclick="window.toggleDayLock(${i})" style="font-size:10px; cursor:pointer; background:${activity.dayLocks[i]?'#fa5252':'#adb5bd'}; color:white; border:none; border-radius:3px; padding:2px 5px; margin-top:5px;">${activity.dayLocks[i]?'잠금':'마감'}</button>` : ''}
                </th>`).join('')}
              <th style="padding:15px; width: 100px;">잔고</th>
            </tr>
          </thead>
          <tbody>
          ${db.globalStudents.map((s: any) => {
            ...
            return `
              <tr style="border-bottom: 1px solid #eee; text-align:center;">
                ...
                <td style="padding:12px; font-size: 0.85rem;">${s.role}<br><small style="color:#888;">(${s.pay.toLocaleString()}원)</small></td>
                ${DAYS.map((_, dIdx) => {
                ...
                <td style="padding:12px; font-weight:bold; color:#1c7ed6;">${s.balance.toLocaleString()}</td>
              </tr>`;
          }).join('')}
          </tbody>
        </table>
...
function renderLogin() {
  app.innerHTML = `
    <div style="padding:100px; text-align:center;">
      ...
      <div style="max-width:300px; margin:auto; background:#f8f9fa; padding:30px; border-radius:15px; border:1px solid #dee2e6;">
        ...
        <input id="l-pw" type="password" placeholder="비밀번호 (학생 1234)" style="padding:12px; width:100%; margin-bottom:20px; border:1px solid #ddd; border-radius:5px; box-sizing:border-box;"><br>
        <button id="l-btn" style="padding:12px; width:100%; background:#228be6; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">로그인</button>
      </div>
    </div>`;
  ...
  document.querySelector('#l-btn')?.addEventListener('click', () => {
    ...
    if (id === 'admin' && pw === '1234') { currentUser = { name: '선생님', isAdmin: true }; }
    else {
      const s = db.globalStudents.find((x:any) => x.name.trim() === id);
      if (s && (pw === '1234' || pw === (id + "123"))) { currentUser = { ...s, isAdmin: false }; }
      else { alert('정보 확인!'); return; }
    }
    sessionStorage.setItem('econ_user', JSON.stringify(currentUser));
    render();
  });
}
    ...
    <div style="background: #fff9db; padding: 20px; border-radius: 12px; border: 1px solid #fab005; display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
      <div style="background:white; padding:15px; border-radius:8px;">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:5px; margin-bottom:10px;">
          <input id="edit-dept" placeholder="부서" style="width:100%;">
          <input id="edit-role" placeholder="직업" style="width:100%;">
          <input id="edit-pay" type="number" placeholder="주급" style="width:100%;">
        </div>
        <div style="display:flex; gap:5px;">
          <button id="edit-s-btn" style="flex:1; background:#fd7e14; color:white; border:none; padding:8px; border-radius:5px; cursor:pointer;">수정</button>
          <button id="set-manager-btn" style="flex:1; background:#495057; color:white; border:none; padding:8px; border-radius:5px; cursor:pointer;">대표설정</button>
      ...
      <div style="background:white; padding:15px; border-radius:8px; text-align:center;">
        ...
        <div style="display:flex; justify-content:center; gap:10px; font-size:0.8rem; margin-top:20px;">
          ...
          <button id="logout-btn" style="color:red;">로그아웃</button>
        </div>
      </div>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; background:#e7f5ff; padding:20px; border-radius:12px; border:2px solid #339af0;">
      <div>
        ...
        <span style="background:${my?.isManager?'#e67e22':'#339af0'}; color:white; padding:2px 8px; border-radius:10px; font-size:0.8rem;">
          ${my?.isManager ? '⭐ 대표 학생 (업무 체크 권한 있음)' : '일반 학생'}
        </span>
      </div>
      <button id="logout-btn" style="padding:10px 20px; cursor:pointer;">로그아웃</button>
    </div>`;
}

(window as any).toggleDayLock = (idx: number) => {
  db = getDB(); const activity = getWeeklyActivity(currentView, db);
...
function setupEvents() {
  document.querySelectorAll('#logout-btn').forEach(b => b.addEventListener('click', () => { 
    currentUser = null; sessionStorage.removeItem('econ_user'); render(); 
  }));
  ...
  document.querySelector('#edit-s-btn')?.addEventListener('click', () => {
    ...
    if (sIdx !== "" && newDept && newRole && newPay) {
      ...
      saveData(); render(); alert('정보 수정 완료!');
    }
  });
  ...
  document.querySelector('#delete-s-btn')?.addEventListener('click', () => {
    const sIdx = (document.querySelector('#edit-s-idx') as HTMLSelectElement).value;
    if (sIdx !== "" && confirm('정말 삭제합니까?')) { db.globalStudents.splice(parseInt(sIdx), 1); saveData(); render(); }
  });
...
function saveData() { localStorage.setItem('econ_v17_db', JSON.stringify(db)); }
render();