...
function renderAdminSection(monthData: any) {
  ...
  return `
    <div style="background: #fff9db; padding: 20px; border-radius: 15px; border: 1px solid #fab005; margin-bottom: 20px;">
      <h4 style="margin-top:0;">🛠️ 관리자 도구</h4>
      <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
        <div style="background:white; padding:15px; border-radius:10px;">
          <p><b>1. 직업 사전 등록</b></p>
          <input id="new-dept" placeholder="부서">
          <input id="new-role" placeholder="직업">
          <input id="new-pay" type="number" placeholder="주급">
          <button id="save-role-btn">목록 추가</button>
        </div>
        <div style="background:white; padding:15px; border-radius:10px;">
          <p><b>2. 학생 등록</b></p>
          ...
          <select id="role-select">
            ...
            ${roles.map((r:any, i:number) => `<option value="${i}">${r.role}</option>`).join('')}
          </select>
          <button id="add-student-btn">추가 완료</button>
        </div>
      </div>
      <div style="display:flex; gap:10px; justify-content: center; margin-top:20px;">
        <button id="reset-week-btn" style="background:#228be6; color:white; padding:10px; border:none; border-radius:8px; cursor:pointer;">💰 주간 정산</button>
      </div>
    </div>
  `;
}
...
function setupEvents() {
  ...
  document.querySelector('#add-student-btn')?.addEventListener('click', () => {
    ...
    if (name && roleIdx) {
      ...
      mData.students.push({
        name, role: r.role, dept: r.dept, roleDetail: r,
