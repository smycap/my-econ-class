            ...
            <tr>
              ...
              ${DAYS.map((d, i) => `
                <th style="padding:10px; width: 80px;">
                  ${d}<br>
                  ${currentUser.isAdmin ? `<button onclick="window.toggleDayLock(${i})" style="font-size:10px; cursor:pointer; background:${activity.dayLocks[i]?'#fa5252':'#adb5bd'}; color:white; border:none; border-radius:3px;">${activity.dayLocks[i]?'잠금':'해제'}</button>` : ''}
                </th>`).join('')}
                ...
                ${DAYS.map((_, dIdx) => {
                  ...
                  <td style="padding:12px;">
                    <button onclick="window.toggleCheck('${s.name}', ${dIdx})" ${!canEdit ? 'disabled' : ''} 
                      style="border-radius:50%; border:none; background:${checks[dIdx] ? '#4caf50' : '#ff4757'}; color:white; width:30px; height:30px; cursor:${canEdit?'pointer':'default'}; opacity:${isLocked?0.5:1};">
...
function renderLogin() {
  app.innerHTML = `
    <div style="padding:100px; text-align:center;">
      ...
      <div style="max-width:300px; margin:auto; background:#f8f9fa; padding:30px; border-radius:15px; border:1px solid #dee2e6;">
        <label style="display:block; text-align:left; font-size:0.8rem; color:#666; margin-bottom:5px;">학생 이름 또는 admin</label>
        <input id="l-id" placeholder="이름을 입력하세요" style="padding:12px; width:100%; margin-bottom:10px; border:1px solid #ddd; border-radius:5px; box-sizing:border-box;">
        <label style="display:block; text-align:left; font-size:0.8rem; color:#666; margin-bottom:5px;">비밀번호 (학생은 숫자 1234)</label>
        <input id="l-pw" type="password" placeholder="비밀번호" style="padding:12px; width:100%; margin-bottom:20px; border:1px solid #ddd; border-radius:5px; box-sizing:border-box;">
  ...
  document.querySelector('#l-btn')?.addEventListener('click', () => {
    ...
    // 2. 학생 로그인 (아이디: 이름 / 비번: 1234 로 통일)
    const s = db.globalStudents.find((x:any) => x.name.trim() === id);
    if (s) {
      if (pw === '1234' || pw === (id + "123")) {
        currentUser = { ...s, isAdmin: false };
        render();
      } else {
        alert('비밀번호가 틀렸습니다. (학생 비밀번호는 1234입니다)');
      }
    } else {
      alert('등록되지 않은 이름입니다. 이름을 확인해 주세요!');
    }
  });
}
  ...
  return `
    <div style="background: #fff9db; padding: 20px; border-radius: 12px; border: 1px solid #fab005; display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
      <div style="background:white; padding:15px; border-radius:8px;">
        <p style="margin:0 0 10px 0;"><b>👤 학생 관리</b></p>
        ...
        <div style="display:flex; gap:5px;">
          <button id="set-manager-btn" style="flex:1; background:#495057; color:white; border:none; padding:8px; border-radius:5px; cursor:pointer;">대표지정/해제</button>
          <button id="delete-s-btn" style="flex:1; background:#fa5252; color:white; border:none; padding:8px; border-radius:5px; cursor:pointer;">학생 삭제</button>
        </div>
      </div>
      <div style="background:white; padding:15px; border-radius:8px; text-align:center;">
        <p style="margin:0 0 10px 0;"><b>🛠️ 정산 및 도구</b></p>
        ${!activity.isPaid 
...
function renderStudentSection() {
  ...
  return `
    <div style="display:flex; justify-content:space-between; align-items:center; background:#e7f5ff; padding:20px; border-radius:12px; border:2px solid #339af0;">
      <div><h2>내 잔고: ${my?.balance.toLocaleString()}원</h2><span>${my?.isManager ? '⭐ 대표 학생 권한 보유' : '일반 학생'}</span></div>
      <button id="logout-btn">로그아웃</button>
    </div>`;
}
...
function setupEvents() {
  ...
  document.querySelector('#set-manager-btn')?.addEventListener('click', () => {
    ...
    if (sIdx !== "") { db.globalStudents[parseInt(sIdx)].isManager = !db.globalStudents[parseInt(sIdx)].isManager; saveData(); render(); }
  });
